---
name: Lab Submission
about: Submit your security incident lab analysis
title: '[SUBMISSION] Ahmet Talha Can - Security Incident Lab'
labels: submission
assignees: ''
---

## 👤 Candidate Information

**Full Name:** Ahmet Talha Can
**Email:**  cantalha4141@gmail.com
**LinkedIn:** (https://www.linkedin.com/in/ahmet-talha-can/)
**Location:** Kocaeli
**Submission Date:** 10.11.2025

---

## 📎 Submission Files

**Option A: Attached Files**
- Report PDF: [Attach here]

**Option B: External Links**
- Video: (https://www.loom.com/share/42723a378de348ecbf936300863ca5af)

---

## ⏱️ Time Tracking

**Total time spent:** _15__ hours

**Breakdown:**
- Log analysis: _4__ hours
- Architecture design: _3__ hours
- Report writing: _6__ hours
- Video creation: _2__ hours

---

## 🎯 Summary

### Attack Vectors Identified
1. API - Broken Object Level Authorization
2. Web - SQL Injection (WAF Bypass)
3. Email - Phishing

### Key Findings
- Authorization checks validate token but may not verify account ownership.
- Identity vulnerability due to lack of mandatory MFA
- Critical SQL Injection bypassed by WAF

### Top 3 Recommendations
1. Require prepared statements in application code
2. Require multi-factor authentication (MFA) for all critical accounts
3. Deploy the OLA module that enforces authorization

---

## 🛠️ Tools Used

**Analysis:**
- Web Browsers
- Excel

**Diagrams:**
- Draw.io

**Video:**
- Loom

**Other:**
- 

---

## ✅ Checklist

Please confirm:

- [x] Report is max 5 pages
- [x] Video is 10-15 minutes
- [x] All log files were analyzed
- [x] Timeline is timezone-corrected
- [x] Framework mappings included (ISO 27001, NIST, OWASP)
- [x] Architecture diagram included
- [x] Video link is tested and working
- [x] No plagiarism / proper attribution
- [x] Original work, not AI-generated

---

## 💬 Optional Feedback

**What did you think of this lab?**
It was a wonderful experience for me. I learned how to write reports during my internship, but this was the first time I saw and analyzed such a detailed scenario. It was very enjoyable. Besides participating to apply for a job, I'd love to participate in labs like this again at a later time.

**Any suggestions for improvement?**


**Would you recommend this to others?**
- [x] Yes
- [ ] No
- [ ] Maybe

---

## 📧 Contact Preference

**Preferred contact method:**
- [x] Email
- [x] LinkedIn
- [ ] GitHub

**Best time to reach you:**
Everyday after 10am

---

## ⚖️ Declaration

I declare that this submission is my original work and I have followed all guidelines.

**Name:** Ahmet Talha Can
**Date:** 10.11.2025

---

_Thank you for your submission! We'll review it and get back to you within 1 week._
